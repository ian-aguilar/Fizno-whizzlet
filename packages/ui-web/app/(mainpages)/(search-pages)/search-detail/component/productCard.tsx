...
export default function ProductCard({ type, product }: ProductCardProps) {
  return (
    <div
      className={`product_card border-2 p-3 rounded-lg relative ${type === "listView" ? "flex w-full" : "block"}`}
    >
      <div className="absolute right-2">
        <span className="text-yellow-600 w-5 h-5 cursor-pointer">
          <SVG.favouriteIcon />
          <SVG.favouriteFillIcon />
        </span>
      </div>
      <div
      ...
      <div className="py-2 w-full">
        ...
        >
          ...
          >
            <button
              className={`text-[11px] bg-primaryMain text-white font-semibold border border-blueTwo h-9 px-2  rounded-md ${type === "listView" ? "w-full" : "w-auto"}`}
            >
              Buy Now
            </button>
            {/* <div className="flex justify-end"> */}
            <button
              className={`text-[11px] whitespace-nowrap  bg-transparent text-primaryMain font-semibold border px-2 border-blueTwo h-9 rounded-md ${type === "listView" ? "w-full mt-4" : "w-auto"}`}
            >
