...
import { Content } from "./footer-table";
...
export default function FooterSocialMediaTableItem({
  category,
  onCheckboxChange,
  isSelected,
  snackbar,
  ...
  const deleteItem = async () => {
    try {
      const response = await AdminApi.deleteSocialMediaIcon(category.id);
  ...
  return (
    <tr>
      ...
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div className=" font-bold text-primaryMain">{category.url}</div>
      </td>
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
        ...
        <button
          ...
          onClick={() =>
            router.push(`/manage-footer/edit-social-media?id=${category.id}`)
          }
        >
          <svg
            stroke="currentColor"
