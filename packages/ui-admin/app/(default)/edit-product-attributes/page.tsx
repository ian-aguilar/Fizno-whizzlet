...
export default function AddProductAttributes() {
  ...
  const OrderBy = [
    { value: "menu_order", label: "Menu ordering" },
  ...
  const formik = useFormik<AttributesValue>({
    ...
    onSubmit: async (
      ...
      setSubmitting(true);
      const response = await categoriesService.updateAttributesById(
        values,
        id as string,
      );
  ...
  const fetchCategories = async () => {
    const response = await categoriesService.getAttributesById(id as string);
    if (response.remote === "success") {
      console.log(response);
      setAttribute(response.data.data.data);
  ...
  useEffect(() => {
    ...
    if (attribute) {
      formik.setValues({
        ...
        archive: attribute.attribute_public == 0 ? true : false,
        ordering: "",
        terms: [],
      });
      if (attribute.details) {
        setTags(
          attribute.details?.map((item: any) => {
        ...
        >
            ...
            <div className="w-6/12">
              ...
              <div className="mt-4 ">
                ...
                <input
                  type="checkbox"
                  onChange={(e) =>
                    formik.setFieldValue("archive", e.target.checked)
                  }
                  checked={formik.getFieldProps("archive").value}
          ...
          <div className="mt-4">
            ...
            <TagInput
              ...
              hideRemove={true}
              onRemoveTag={(data) => console.log(data)}
            />
