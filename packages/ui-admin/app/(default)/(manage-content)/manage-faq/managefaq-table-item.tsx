...
import { Content } from "./managefaq-table";
import { AdminApi } from "@fizno/api-client/src/apis/AdminApi";

interface ManageFaqTableItemProps {
  category: Content;
...
export default function ManageFaqTableItem({
  category,
  onCheckboxChange,
  isSelected,
  snackbar,
  setSnackbar,
}: ManageFaqTableItemProps) {
  ...
  return (
    <tr>
      ...
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
        <div className=" font-bold text-primaryMain">{category.question}</div>
      </td>
      ...
      <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap w-px">
        ...
        <button
          ...
          onClick={() => router.push(`/edit-faq?id=${category.id}`)}
        >
          <svg
            stroke="currentColor"
            fill="none"
