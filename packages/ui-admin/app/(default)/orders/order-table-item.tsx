      ...
      <tr>
        ...
        <td className="px-2 first:pl-5 last:pr-5 py-3   w-px">
          <div className="text-center">
            ...
            />
            <div className=" w-40">
              {order.orderItems[0]?.product.post_title}
            </div>
          </div>
        </td>
        <td className="px-2 first:pl-5 last:pr-5 py-3 whitespace-nowrap">
          <div className=" text-center">
            <div className="font-medium text-slate-800 dark:text-slate-100">
              #{order.orderId} by {order.orderItems[0]?.user?.display_name}
            </div>
            <div className="text-center font-medium text-fuchsia-500">
              {order.orderItems.map((item: any) => {
                return `${item.product_qty} X ${item.product.post_title.slice(0, 20)}...`;
              })}
            </div>
            <p
