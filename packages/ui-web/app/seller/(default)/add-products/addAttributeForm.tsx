...
export default function AddAttributeForm() {
  ...
  const attributeArray = [
    { label: "attributeOne", value: "Attribute One" },
    { label: "attributeTwo", value: "Attribute Two" },
  ];
  ...
  const handleAddAttributeCard = () => {
    // Push a new attribute object with default values to the attributes array
  ...
  return (
    <div>
      ...
      {attributes.map((attribute, index) => (
        <div
          key={index}
          className="px-4 py-3 rounded-sm dark:bg-slate-800 border border-slate-200 dark:border-slate-700 mb-3"
        >
          {open ? (
            <>
              {" "}
      ...
      <div className="w-6/12 mt-7">
        <div className="flex items-center">
          <div className="w-full">
            <SelectComponent label="" options={attributeArray} />
          </div>{" "}
          <span
            className="ms-3 bg-primaryMain hover:bg-blueTwo py-0 h-[35px] px-3 text-white rounded-md"
            onClick={handleAddAttributeCard}
          >
            Add
          </span>
        </div>
