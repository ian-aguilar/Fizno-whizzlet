...
export default function EditCoupons() {
  ...
  const formik = useFormik<DiscountCouponType>({
    ...
    onSubmit: (values) => {
      handleUpdateDiscountCoupon(values);
    },
  });
  ...
  const handleUpdateDiscountCoupon = async (values: DiscountCouponType) => {
    const payload = {
      data: {
    ...
    setLoading(true);
    const response = await UserApi.updateDiscountCouponAPI(payload);
  ...
  /**
   * handle get coupon details
   */

  const handleGetCouponDetails = async () => {
    if (couponId) {
      ...
      if (response.remote === "success") {
        ...
        formik.setFieldValue(
          "discountType",
          discountTypeArray.find((el) => el.value === data.coupon_type),
        );
        formik.setFieldValue("couponAmount", data.coupon_amount);
        formik.setFieldValue("couponExpiry", data.expire_at);
        formik.setFieldValue("description", data.description);
        formik.setFieldValue("minSpend", data.minimum_spent);
        formik.setFieldValue("maxSpend", data.maximum_spent);
        formik.setFieldValue("limitPerCoupon", data.limit);
        formik.setFieldValue("limitPerItem", data.limit_per_item);
        formik.setFieldValue("limitPerUser", data.limit_per_user);
      }
    }
  };
